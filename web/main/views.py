            ...
            if name and message and email_address:
                ...
                email_message = EmailMessage(
                    ...
                    to=[
                        ...
                        "jimmywainaina@gmail.com",
                        "thomas.muteti@gmail.com",
                ...
                context = {"title": title, "helpmessage": helpmessage, "showform": showform}
...
def data(request):
    ...
    if not request.user.is_superuser:
        shambas = serialize(
            "geojson", shambas, fields=[f.name for f in Shamba._meta.fields if f.name != "owner"]
        )
    else:
        shambas = serialize("geojson", shambas)
...
def registershamba(request):
    ...
    context = {"title": title, "reg_form": reg_form}
    if request.method == "POST":
        pass
    else:
        return render(request, template, context)
    ...
    if request.method == "POST":
        ...
        if post_type == "yearly-update":
            ...
            if request.user.is_staff:
                ...
                message = "You need to be staff to take that action"
    context = {"shambas": shambas, "message": message}
    return render(request, template, context)
